...
export function ExpensesPage() {
  ...
  const categories: ExpenseCategory[] = ['Bahan', 'Operasional', 'Transport', 'Promosi', 'Lainnya'];
  ...
  const monthSummary = useMemo(() => {
    ...
    return { total, breakdown, count: monthExpenses.length };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      refreshData().catch(err => console.error('Error refreshing after expense save:', err));
    } catch (err: any) {
      console.error('Error saving expense:', err);
      setToast({
        message: err?.message || 'Gagal menyimpan pengeluaran ke Supabase',
        type: 'error',
      });
    } finally {
  ...
  const handleDeleteExpenseConfirm = async () => {
    ...
    try {
      ...
      refreshData().catch(err => console.error('Error refreshing after expense delete:', err));
    } catch (err: any) {
      console.error('Error deleting expense:', err);
      setToast({
        message: err?.message || 'Gagal menghapus pengeluaran dari Supabase',
        type: 'error',
      });
    } finally {
      setIsDeleting(false);
    ...
    <div className="space-y-6 pb-20 md:pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="p-4 rounded-xl bg-[#101013] border border-[#22222A]">
          ...
          <p className="text-xs text-[#7A7A84]">{monthSummary.count} transaksi biaya</p>
        </div>
        ...
        {categories.slice(0, 3).map(cat => {
          ...
          return (
            <div key={cat} className="p-4 rounded-xl bg-[#101013] border border-[#22222A]">
              <div className="flex items-center justify-between text-xs text-[#7A7A84] mb-2 font-medium">
                <span>Kategori {cat}</span>
                <span className="text-[#F0F0F2] font-semibold">{pct}%</span>
              </div>
      ...
      {isLoadingData && expenses.length === 0 ? (
        ...
        <div className="p-12 text-center rounded-xl bg-[#101013] border border-[#22222A]">
          ...
          <h3 className="text-sm font-semibold text-[#F0F0F2]">Tidak ada catatan pengeluaran</h3>
          ...
          <table className="w-full text-left text-xs">
            <thead className="text-[#7A7A84] border-b border-[#22222A] font-medium">
              <tr>
                ...
                <th className="pb-3 px-3">Nama Pengeluaran</th>
            ...
            <tbody className="divide-y divide-[#22222A]/60">
              {filteredExpenses.map(exp => (
                <tr key={exp.id} className="hover:bg-[#16161B]/40 transition-colors">
      ...
      >
        <div className="space-y-4">
          <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-xs text-rose-300">
            Apakah Anda yakin ingin menghapus catatan pengeluaran <strong>&quot;{deletingExpense?.name}&quot;</strong> senilai <strong>{formatRupiah(deletingExpense?.amount || 0)}</strong> ({deletingExpense?.category})? Data akan dihapus secara permanen dari Supabase.
          </div>
